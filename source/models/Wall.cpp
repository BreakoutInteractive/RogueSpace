...
void EnergyWall::loadAssets(const std::shared_ptr<cugl::AssetManager> &assets){
    ...
    auto floorArrowSheet = SpriteSheet::alloc(assets->get<Texture>("floor-arrows"), 8, 6);
    _currAnimation = Animation::alloc(energyWallSheet, 1.0f, true, animationIndex * 6, (animationIndex+2) * 6 - 1);
    _arrowAnimation = Animation::alloc(floorArrowSheet, 1.0f, true, animationIndex * 6, (animationIndex + 2) * 6 - 1);
}
