 ...
 */
void RocketModel::setShipNode(const std::shared_ptr<cugl::scene2::SceneNode>& node,
    ...
    if (_mainBurner == nullptr) {
      _mainBurner = scene2::SpriteNode::allocWithSheet(assets->get<Texture>(_mainTexture),
                                              1, FIRE_FRAMES, FIRE_FRAMES);
    }
    if (_leftBurner == nullptr) {
      _leftBurner = scene2::SpriteNode::allocWithSheet(assets->get<Texture>(_leftTexture),
                                              1, FIRE_FRAMES, FIRE_FRAMES);
    }
    if (_rghtBurner == nullptr) {
      _rghtBurner = scene2::SpriteNode::allocWithSheet(assets->get<Texture>(_rightTexture),
                                              1, FIRE_FRAMES, FIRE_FRAMES);
    }
 ...
 */
const std::shared_ptr<cugl::scene2::SpriteNode>& RocketModel::getBurnerStrip(Burner burner) const {
    switch (burner) {
        case Burner::MAIN:
            return _mainBurner;
        case Burner::LEFT:
 ...
 */
void RocketModel::setBurnerStrip(Burner burner, const std::string strip) {
	switch (burner) {
	case Burner::MAIN:
		_mainTexture = strip;
		break;
