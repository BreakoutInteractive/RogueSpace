...
Uint32 AudioEngine::_readsize = 0;
...
bool AudioEngine::start(Uint32 slots) {
    ...
    if (_readsize == 0) {
        _readsize = AudioDevices::get()->getReadSize();
    } else {
 ...
 *
 * If the engine is not yet initialized, this value will stored until
 * such time as the engine become active. Otherwise, it will pause the
 * output device and apply the new value recursively to the audio graph.
 *
 * @param size  The read size of this output node.
 */
void AudioEngine::setReadSize(Uint32 size) {
    if (_gEngine == nullptr) {
        _readsize = size;
    } else {
        _gEngine->_output->lock();
        _gEngine->_output->setReadSize(size);
        _gEngine->_output->unlock();
    }
}
...
bool AudioEngine::play(const std::string key, const std::shared_ptr<Sound>& sound,
    ...
    for(auto it = _slots.begin()+1; audioID == -1 && it != _slots.end(); ++it) {
        if (!(*it)->isPlaying() && !(*it)->getTailSize()) {
            audioID = (*it)->getTag();
        }
    }
...
void AudioEngine::clear(const std::string key,float fade) {
    ...
    if (_actives.find(key) != _actives.end()) {
        ...
        // Act only if we are not already fading out
        if (fade == 0) {
            _slots[node->getTag()]->skip();
        } else if (!node->isFadeOut()) {
